...
import vue from '../assets/img_vue.jpg'
import react from '../assets/react.png'

const skillsData = [
  { id: 1, img: vue, title: 'Vue.js', description: 'A progressive JavaScript framework.' },
  { id: 2, img: react, description: 'A JavaScript library for building user interfaces.' },
];
...
const SkillBox = ({ img, title, description }) => (
  <article className='bg-gray-800 p-4 sm:p-6 rounded-lg shadow-lg text-center hover:bg-purple-800 transition-all duration-300'>
    <figure className='flex justify-centermb-4'>
      <img src={img} alt={title || 'Skill Image'} className='w-16 h-16 sm:w-20 sm:h-20'/>
    </figure>
    <header>
...
export default function Skills() {
  return (
    <section id='skills' className='relative min-h-screen overflow-hidden flex flex-col items-center justify-center text-whit px-4 py-10'>
      ...
      <div data-aos="fade-up" data-aos-delay='300' className='relative z-20 text-center space-y-6 sm:space-y-10'>
        ...
        <section data-aos="fade-up" data-aos-delay='500' className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6, mt-4'>
            {skillsData.map((skill) => (
              <SkillBox key={skill.id} img={skill.img} title={skill.title} description={skill.description} />
            ))}
          </section>
      </div>
